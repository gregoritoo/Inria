...
#Importation des données et de la libraire randomForest. On enlève les variables inutiles et redondantes ( pas besoin d'avoir start time et stop time quand on a runtime)
...
#On utilise le modèle pour prédire sur le jeu d'entrainement.La précision est correcte : 96,7%
...
#Sur cet exemple, on remarque que la précision augmente jusqu'à ce que l'on prenne 5 variables, en compte. Elle descent ensuite pour 7 puis remonte évidemment si l'on prend en compte toutes les variables. Ainsi, il semble être optimal de prendre 5 variables, qui sont celles présentées auparavant :
 #- run_time_oar
 # - job_user_oar
 # - submission_time_oar 
 # - job_id_oar
...
plot(3:10,Précisions,type='l',xlab ='nombre de variables')
